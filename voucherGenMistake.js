...
function getVoucherDetails(code, phone, outlet_name) {
	...
	Voucher.findOne({'basics.code': code})
        ...
        if(err || voucher === null) {
        	...
        	var push_message = 'You have unlocked a reward at '+outlet_name+'. Your voucher code is '+voucher.basics.code+'. '+CommonUtilities.rewardify(voucher.issue_details.issued_for)+', valid '+getCheckinApplicabilityDay(voucher.issue_details.issued_for.reward_applicability.day_of_week)+', '+getCheckinApplicabilityTime(voucher.issue_details.issued_for.reward_applicability.time_of_day)+', until '+CommonUtilities.formatDate(new Date(voucher.validity.end_date))+'. ' +(voucher.issue_details.issued_for.terms ? 'Terms- '+voucher.issue_details.issued_for.terms : '')+ 'To claim, please show this SMS to the outlet staff on your NEXT VISIT.';
