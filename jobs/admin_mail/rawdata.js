...
module.exports.execAsyncCommands = function(main_callback) {
	...
	async.series({
	    accounts: function(callback) {
	    	exec("mongoexport --host 50.112.253.131:27017 --db twyst --collection accounts --csv --fields username,email,role,phone,gcm,_id,created_at -q '{created_at:{$gte:new Date(1425148200000)}}' --out ./data/accounts.csv", function (err, result) {
	    		if(err) {
	    			console.log(err	)
	    ...
	    checkins: function(callback) {
	    	exec("mongoexport --host 50.112.253.131:27017 --db twyst --collection checkins --csv --fields phone,outlet,checkin_code,checkin_type,created_date -q '{created_date:{$gte:new Date(1425148200000)}}' --out ./data/checkins.csv", function (err, result) {
	    		if(err) {
	    			return callback(err, null);
	    		}
	    		callback(null, result);
