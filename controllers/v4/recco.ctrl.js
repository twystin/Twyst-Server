...
var HttpHelper = require('../../common/http_helpers');
    ...
    if (!outlets || !outlets.length) {
      ...
      getProgramsForOutlets(outlets, function(objects) {
        getIndependentUserData(function(results) {
          ...
          AuthCtrl.logged_in(req.query.token, function(user) {
            if (user) {
              getUserHistory(user, function(err, history) {
  ...
  if (!cut_reccos || !cut_reccos.length) {
    ...
    getRewards(cut_reccos, function(err, rewards) {
      if (err) {
        ...
        cb(null, getReccosWithMatchedRewards(cut_reccos, rewards));
      }
    })
  }
}

function getReccosWithMatchedRewards(cut_reccos, rewards) {
...
function getCheckinRelevance(checkins, outlet, program) {
  ...
  for (var i = 0; i < checkins.length; i++) {
    if (checkins[i]._id && checkins[i]._id.outlet && checkins[i]._id.outlet.equals(outlet._id)) {
      checkin_on_outlet += checkins[i].count;
    }
    if (program && checkins[i]._id && checkins[i]._id.program && checkins[i]._id.program.equals(program._id)) {
...
function getActiveVouchersHere(rewards, program) {
  ...
  for (var i = 0; i < rewards.length; i++) {
    if (rewards[i].issue_details && rewards[i].issue_details.program && rewards[i].issue_details.program.equals(program._id)) {
